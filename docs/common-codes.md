...
공통 편집 컴포넌트는 `packages/ui/src/CommonCodes.tsx`에 있습니다. BO와 독립 Admin 서비스가 각자의 API 함수를 전달하며 서로의 앱 소스를 가져오지 않습니다. Admin은 `/api/v1/admin/code-groups`와 관리자 전용 세션을 사용하고, 아래 BO의 기존 권한 체계도 유지됩니다.
