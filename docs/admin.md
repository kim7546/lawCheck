...
`apps/admin` 독립 서비스에서 운영 요약, 질문현황, 통계, 사용자 관리, 공통코드 관리, BO 메뉴 관리를 제공합니다. 로컬 주소는 `http://127.0.0.1:5175/`입니다. FO·BO와 별도로 실행·빌드·배포하고 공통 Backend API와 DB를 사용합니다.
...
- 메뉴 순서는 운영 요약 → 질문현황 → 통계 → 사용자 관리입니다. 직접 주소는 `/#questions`, `/#statistics`입니다.
- 두 화면에서 연도 전체 또는 시작일·종료일을 선택합니다. 기본값은 한국 시간 기준 올해 전체이며, 종료일도 포함합니다. 여러 연도에 걸친 일자 범위도 조회할 수 있습니다.
- 기간은 질문의 접수일 기준입니다. 질문현황은 최신순으로 20건씩 질문 원문, 처리 상태, 연결된 AI 답변·비법률 안내 및 완료된 전문가 답변 전체를 제공합니다. 답변 작성일이 조회 기간 밖이어도 질문과 함께 표시합니다. 답변 생성 실패·처리 중 질문도 빠뜨리지 않습니다.
- `/api/v1/admin/questions`, `/api/v1/admin/question-statistics`는 다른 관리 API와 동일한 활성 관리자 인증을 요구합니다.
- 현재 저장된 질문에는 주제값이 없으므로 질문 키워드로 자동 분류합니다. 비법률 안내 답변이 있으면 비법률 질문으로, 그 외에는 부동산·임대차 → 노동·직장 → 가사·생활 → 형사·범죄 → 사업·세무 → 민사·금전 순서의 첫 일치 항목으로 분류합니다. 일치하지 않으면 기타·미분류입니다. 문맥을 해석하는 AI 분류가 아니므로 복합 주제·후속 질문의 분류에는 한계가 있습니다.
- 기존 질문에도 같은 규칙을 적용하며 별도 AI 호출·분류 데이터 변경은 없습니다. 통계는 답변 개수와 무관하게 질문 한 건을 한 번만 집계하고, 주제별 질문 수와 전체 대비 비중을 표시합니다.
- Migration `202609260002_admin_questions`는 기간 조회용 인덱스만 추가하며 기존 질문·답변을 변경하지 않습니다. 배포 전 `npm run db:migrate`를 실행합니다.
